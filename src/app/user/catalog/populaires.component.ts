      ...
      <header class="site-header">
        <div class="site-header__inner">
          ...
          <nav class="site-nav">
            ...
            <a class="site-nav__link site-nav__link--active" routerLink="/bibliotheque/populaires">Populaires</a>
      ...
      <section class="hero">
        <div class="hero__inner">
          <div class="hero__content">
            <span class="badge">Populaires</span>
            <h1>Les manuels les plus consultés et recommandés par la communauté EDIG</h1>
            <p>Découvrez les références incontournables : ressources très demandées, validées sur le terrain et prêtes à être utilisées.</p>
            <a routerLink="/bibliotheque/collection" class="btn btn--primary">Explorer le catalogue complet</a>
          </div>
        ...
        <div class="list__inner">
            ...
            <ng-container *ngIf="filteredManuals.length; else emptyState">
              <div class="cards">
                <article class="card" *ngFor="let m of filteredManuals; trackBy: trackByManualId">
                  <div class="card__image" [style.backgroundImage]="getCardBackground(m)" role="presentation"></div>
                  <div class="card__body">
                    <div class="card__tag">Populaire</div>
          ...
          <ng-template #emptyState>
            <div class="empty-state">
              <h3>Aucun manuel populaire pour le moment.</h3>
              <p>Activez l’option « Populaire » lors de l’ajout d’un manuel pour le faire apparaître dans cette sélection.</p>
            </div>
          </ng-template>
        </div>
      </section>
...
export class PopulairesComponent implements OnInit, AfterViewInit, OnDestroy {
  readonly fallbackImage =
    'https://images.unsplash.com/photo-1520294222220-d8a5b8c6d806?auto=format&fit=crop&w=600&q=80';
  ...
  async ngOnInit(): Promise<void> {
    await Promise.all([this.loadPopularManuals(), this.loadLevels()]);
  ...
  private async loadPopularManuals(): Promise<void> {
    try {
      ...
      console.error('Erreur inattendue lors du chargement des populaires :', err);
      this.manuals = [];
    }
  }
  ...
  private updateDerivedData(): void {
    ...
    if (term) {
      manuals = manuals.filter((m) =>
        [m.title, m.author, m.publisher, m.subject, m.description].map((v) => (v ?? '').toLowerCase()).join(' ').includes(term),
      );
    }
